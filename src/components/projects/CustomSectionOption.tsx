...
export function CustomSectionOption({
  options, totalLengthMm, totalHeightMm, postProfile, topRailProfile,
  bottomRailProfile, panelHeightMm, sheetThickness, allSheets,
  ...
  const derivedSectionWidth = sections && totalLengthMm
    ? Math.round(totalLengthMm / sections)
    : undefined
  ...
  function mergeAndSync(next: Partial<{
    ...
    const derived = s && totalLengthMm ? Math.round(totalLengthMm / s) : undefined
    onCustomChange({
      custom_sections: s,
  ...
  function handleSections(v: number | undefined) {
    setSections(v)
    if (typeof v === 'number' && totalLengthMm) {
  ...
  function handleSectionWidth(v: number | undefined) {
    ...
    if (typeof v === 'number' && v > 0 && totalLengthMm) {
      setSections(Math.round(totalLengthMm / v))
    }
  ...
  if (hasSections && displaySectionWidth && totalLengthMm) {
    const calc = sections! * displaySectionWidth
    const diff = Math.abs(calc - totalLengthMm)
    if (diff > 5) {
      warnings.push(`Sections × width = ${sections} × ${displaySectionWidth} = ${calc}mm, but total length is ${totalLengthMm}mm`)
    }
  }

  if (displaySectionWidth && displaySectionWidth > 2000) {
    warnings.push(`Section width ${displaySectionWidth}mm exceeds 2000mm — check structural adequacy`)
  }

  const openingW = displaySectionWidth ? displaySectionWidth - postW : 0
  const openingH = panelHeightMm ?? (totalHeightMm - topRailH - bottomRailH)
  const minCutW = openingW + 2 * CATCH_OVERLAP
  const minCutH = openingH + 2 * CATCH_OVERLAP
  if (hasCut && displaySectionWidth && (cutW! < minCutW || cutH! < minCutH)) {
    errors.push(`Cut size (${cutW}×${cutH}mm) is smaller than opening + overlap (${minCutW}×${Math.round(minCutH)}mm)`)
  }
          ...
          <div className="border border-border rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold">
                {matchResult.exact ? 'Exact match' : 'Closest option'}: {matchResult.match.num_sections} sections × {matchResult.match.section_width_mm}mm
              </p>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
              ...
              <span className={matchResult.match.structural ? 'text-green-600' : 'text-red-600'}>
                {matchResult.match.structural ? '✓' : '✗ — exceeds 2000mm'}
              </span>
            </div>
