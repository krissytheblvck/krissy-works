      ...
      <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <Card>
          ...
          <CardContent className="p-0">
            {projects.length === 0 ? (
              <p className="text-sm text-muted px-6 py-8 text-center">No projects yet.</p>
            ) : (
              <table className="w-full text-sm">
                      ...
                      <tr key={p.id} className="hover:bg-background transition-colors">
                        <td className="px-6 py-3">
                          <Link href={`/projects/${p.id}`} className="group">
                            <p className="font-medium text-foreground group-hover:text-blue-600 transition-colors">{p.project_code}</p>
                        ...
                        <td className="px-4 py-3">
                          <Badge className={STATUS_COLORS[p.status as keyof typeof STATUS_COLORS] ?? 'bg-surface-muted text-muted'}>
                ...
                <tfoot>
                  <tr className="border-t-2 border-border bg-background">
                    <td colSpan={3} className="px-6 py-3 text-sm font-semibold text-foreground">Total</td>
                    <td className="px-6 py-3 text-right font-bold text-foreground">{formatCurrency(totalValue)}</td>
                  </tr>
                </tfoot>
              </table>
            )}
